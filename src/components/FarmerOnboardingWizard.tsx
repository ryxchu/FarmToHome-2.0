...
export const FarmerOnboardingWizard: React.FC<FarmerOnboardingWizardProps> = ({
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>, field: 'govId' | 'cert') => {
    ...
    if (file) {
      ...
      reader.onloadend = async () => {
        ...
        try {
          // Compress of size using optimal quality 0.6 and maximum 600px dimension
          const compressed = await compressImage(rawBase64, 600, 600, 0.6);
          if (field === 'govId') {
            setGovIdUrl(compressed);
            setErrors(prev => ({ ...prev, govIdUrl: '' }));
          } else {
            setAlternativeCertUrl(compressed);
            setErrors(prev => ({ ...prev, alternativeCertUrl: '' }));
          }
        } catch (comError) {
          console.warn("Onboarding photo compression failed, using original size:", comError);
          if (field === 'govId') {
            setGovIdUrl(rawBase64);
            setErrors(prev => ({ ...prev, govIdUrl: '' }));
          } else {
            setAlternativeCertUrl(rawBase64);
            setErrors(prev => ({ ...prev, alternativeCertUrl: '' }));
          }
        }
      };
  ...
  const handleSubmit = async () => {
    ...
    try {
      // Direct safety compression on submit to ensure Firestore fits within the 1MB document size limit
      let finalGovId = govIdUrl;
      let finalCert = alternativeCertUrl;

      if (govIdUrl && govIdUrl.startsWith('data:image/')) {
        try {
          finalGovId = await compressImage(govIdUrl, 400, 400, 0.5);
        } catch (err) {
          console.warn("Failed to compress gov ID at submission:", err);
        }
      }

      if (alternativeCertUrl && alternativeCertUrl.startsWith('data:image/')) {
        try {
          finalCert = await compressImage(alternativeCertUrl, 400, 400, 0.5);
        } catch (err) {
          console.warn("Failed to compress certificate at submission:", err);
        }
      }
      ...
      console.error(e);
      alert("Error: " + (e.message || "Hindi maipasa ang impormasyon. Mangyaring subukan muli."));
    } finally {
